...
def generate_coco_from_custom(
    ...
    coco_save_root = save_root if save_root is not None else data_root

    root_split_path = os.path.join(coco_data_root, root_split)
    if is_thermal:
        root_split_path = os.path.join(
            coco_data_root,
            root_split.split('_')[0]
        )
    ...
    coco_orig_category_id_to_sorted_order_dict = {
        1: 1,
        2: 2,
        3: 3,
        4: 4,
    ...
    for anno in annotations['annotations']:
        num_objs_per_image = num_objs_per_image.get(anno["image_id"], 0) + 1
...
if __name__ == '__main__':
    def generate_coco_flir(name: str,
