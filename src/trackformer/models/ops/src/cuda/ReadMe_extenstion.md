...
1. [ **CUDA 병렬 연산 구조: Grid, Block, Thread**](#1-cuda-병렬-연산-구조-grid-block-thread)
    1. [기본 개념](#11-기본-개념)
    2. [병렬 처리 계층 구조](#12-병렬-처리-계층-구조)
        1. [Grid(그리드)](#121-grid-그리드)
        2. [Block(블록))](#122-block-블록)
        3. [Thread(스레드)](#123-thread-스레드)
    3. [커널 실행 구성 (`<<<>>>` 문법)](#13-커널-실행-구성--문법)
    4. [매크로 및 함수](#14-메모리-관리)
    5. [CUDA 스트림](#15-cuda-스트림)
    6. [실전 예제: 벡터 덧셈](#16-실전-예제-벡터-덧셈)
    7. [스트림을 활용한 최적화](#17-스트림을-활용한-최적화)
    8. [고급 활용법](#18-고급-활용법)
2. [**CUDA 스트림과 비동기 실행: 동기화 없이 커널 완료 확인 방법**](#2-cuda-스트림과-비동기-실행-동기화-없이-커널-완료-확인-방법)
    1. [비동기 실행의 특성](#21-비동기-실행의-특성)
    2. [비동기 커널 실행과 동기화](#22-비동기-커널-실행과-동기화)
    3. [`ms_deformable_im2col_cuda` 함수 내에서의 동기화](#23-ms_deformable_im2col_cuda-함수-내에서의-동기화)
    4. [모든 커널 실행 완료 여부 확인 방법](#24-모든-커널-실행-완료-여부-확인-방법)
    5. [요약 및 권장 사항](#25-요약-및-권장-사항)
- [**추가 설명: CUDA의 병렬 처리 계층 구조**](#a-추가-설명-cuda의-병렬-처리-계층-구조)
    1. [Grid, Block, Thread 관계 및 세부 사항](#a1-grid-block-thread-관계-및-세부-사항)
    2. [워프(Warp)와 레지스터(Register)](#a2-warp-레지스터-및-성능-최적화)
    3. [Occupancy (오큐펄런시)](#a3-occupancy-오큐펄런시)
    4. [메모리 접근 패턴 최적화](#a4-메모리-접근-패턴-최적화)
    5. [결론](#a5-결론)
...
- **형태**: 그리드는 1D, 2D, 3D 형태로 구성할 수 있습니다.
- **목적**: 전체 커널 작업을 여러 블록에 나누어 분배합니다.

#### 1.2.2. **Block (블록)**
- **정의**: 하나의 블록 안에 여러 스레드가 포함됩니다.
- **형태**: 블록도 1D, 2D, 3D로 구성할 수 있습니다.
...
### A. 추가 설명: CUDA의 병렬 처리 계층 구조

#### A.1. Grid, Block, Thread 관계 및 세부 사항
